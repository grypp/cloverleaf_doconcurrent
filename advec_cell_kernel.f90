...
CONTAINS
  ...
  SUBROUTINE advec_cell_kernel(x_min,       &
    ...
    IF(dir.EQ.g_xdir) THEN
      ...
      DO CONCURRENT(k=y_min:y_max, j=x_min:x_max+2) &
          LOCAL(upwind,donor,downwind,dif,sigmat,sigma3,sigma4,sigmav,sigma, &
                sigmam, diffuw,diffdw,limiter,wind)
      ...
      DO CONCURRENT(k=y_min:y_max, j=x_min:x_max) &
          LOCAL(pre_mass_s,post_mass_s,post_ener_s,advec_vol_s)
    ...
    ELSEIF(dir.EQ.g_ydir) THEN
      ...
      DO CONCURRENT(k=y_min:y_max+2, j=x_min:x_max) &
          LOCAL(upwind,donor,downwind,dif,sigmat,sigma3,sigma4,sigmav,&
                sigma,sigmam,diffuw,diffdw,limiter,wind)
